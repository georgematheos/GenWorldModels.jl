...
function infer_mean_num_true_facts(tr, num_iters; log_freq=num_iters + 1, save_freq=num_iters+1)
    num_numfacts_sum = 0
    iters_run = 0
    function examine!(tr)
        current_num = length(tr[:kernel => :facts => :facts])
        num_numfacts_sum += current_num
        iters_run += 1
        println("Current num facts: ", current_num)
    end
    # try
        run_inference!(tr, examine!, num_iters; log_freq=log_freq, examine_freq=1, save_freq=save_freq)
        return num_numfacts_sum/iters_run
    # catch e
    #     println("Caught error ", e)
    #     println("Managed to run $iters_run iterations.  Mean num relations at this point was ", num_rel_sum/iters_run)
    # end
...
struct IterationRecord
    ...
    tracker::Union{AccTracker, Nothing}
    additional_information
end

function save!(tr, datetime, i, tracker, additional_information)
    path = joinpath(@__DIR__, "saves", "run_$(datetime)_iteration_record__$i")
    record = IterationRecord(Gen.deep_dynamic_copy(get_choices(tr)), tracker, additional_information)
...
function run_inference!(tr, examine!, num_iters;
    ...
    save_acc_tracker=false,
    get_additional_save_state=((tr,) -> nothing)
)
    ...
    for i=1:num_iters
        ...
        if i % save_freq === 0
            trk = save_acc_tracker ? tracker : nothing
